...
En windows
Para compilar el programa: gcc practicapila.c TADPilaDL.c TADListaDL.c -o practica -lm
...
#include "TADPilaDL.h" // Libreria estatica
...
int main()
{
	...
	InitializeS(&pila_cad); // Se incicializa la pila_cad
	...
	InitializeS(&pila_cad);//Se inicializa la pila_cad
	...
	for(m=0;m<tam_cadena;m++)
	{
		...
		else if(cadena[m]==')')
		{
			...
			while (e1.c!='(')
			{
				...
				e1=PopS(&pila_cad);//Saca el tope de la pila
			...
			e1=PopS(&pila_cad);// Saca el paréntesis que está en el tope de la pila
		...
		else if(cadena[m]=='*')
		{
			...
			if((SizeS(&pila_cad))==0 || e1.c=='+' || e1.c=='-' || e1.c=='(')
			{
	...
	{	
		...
		if(postfijo[n]=='+')
		{
			e3=PopS(&pila_r);//Se hace pop del último elemento que esté en la pila(A,B,C,...) y se mantiene el valor en e3
			e4=PopS(&pila_r);//Se hace pop del último elemento que esté en la pila(A,B,C,...) y se mantiene el valor en e4
			...
			e2.d=e4.d+e3.d; 
			PushS(&pila_r, e2);//El resultado de la operación anterior se agrega a la pila	
			...
			*/
			...
			{				
				...
				if(resultado[n]==0)
				{
					...
					PushS(&pila_r,e2);//Se ingresa el valor a la pila
