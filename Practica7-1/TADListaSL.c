...
a nivel de bytes. 
*/
posicion Search (lista *l,elementoL e)
{	
	posicion aux=l->frente;
	while(aux!=NULL)
	{
		if(memcmp(&aux->e,&e,sizeof(elementoL))==0)
		{
			return aux;			
		}
		aux=aux->siguiente;
...
*/
elementoL Position (lista *l,posicion p)
{
	elementoL e;
	if(ValidatePosition(l,p))
		return p->e;
...
*/
void Insert (lista * l, posicion p, elementoL e, boolean b)
{
...
Recibe: lista *l (Referencia a la lista "l"), elemento e (Elemento a insertar en la lista)
Devuelve: 
Observaciones: El usuario a creado una lista,la lista fue correctamente inicializada.
*/
...
Observaciones: El usuario a creado una lista,la lista fue correctamente inicializada, la posicion p es valida.
*/
void Replace (lista *l,posicion p, elementoL e)
{
